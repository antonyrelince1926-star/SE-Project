...
export const AdminDashboardView: React.FC = () => {
  ...
  const handleUpdateStatus = async (userId: string, currentStatus: string) => {
    if (!isAdmin) {
      showToast('Access Denied: Only administrators have permission to suspend or activate accounts.', 'error');
      return;
    }
  ...
  const handleUpdateRole = async (userId: string, currentRole: string) => {
    if (!isAdmin) {
      showToast('Access Denied: Only administrators have permission to alter user roles.', 'error');
      return;
    }
  ...
  const handleDeleteUser = async () => {
    if (!isAdmin) {
      showToast('Access Denied: Only administrators have permission to delete user accounts.', 'error');
          ...
          {isAdmin ? (
            <>
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 shadow-xs transition-all cursor-pointer"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold text-indigo-700 dark:text-indigo-300 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 transition-all cursor-pointer"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold text-purple-700 dark:text-purple-300 bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/20 transition-all cursor-pointer"
              >
            ...
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium bg-slate-100 dark:bg-slate-800 text-slate-400 border border-slate-200 dark:border-slate-700">
                <Lock className="w-3.5 h-3.5 text-slate-400" />
                <span>Admin Actions Locked (Read-Only)</span>
              </span>
            </div>
          )}
        </div>
                      ...
                      <td className="px-4 py-3 text-right">
                        {isAdmin ? (
                          ...
                          <div className="flex items-center justify-end">
                            <span
                              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-medium bg-slate-100 dark:bg-slate-800/80 text-slate-400 border border-slate-200 dark:border-slate-800 cursor-not-allowed"
                              title="Administrator privilege required to modify or delete accounts"
                            >
